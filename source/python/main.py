...
async def main():
    ...
    device = await BleakScanner.find_device_by_address('90:7B:C6:69:18:3C', timeout=5, cb=dict(use_bdaddr=True))
    if device is None:
        logger.error("could not find device with address '%s'", '90:7B:C6:69:18:3C')
        exit(0)
